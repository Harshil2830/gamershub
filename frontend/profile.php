...
if(isset($_SESSION["username"])){
	...
	$request = array();
	$request['type'] = "displayfriends";
	$request['user_id'] = $_SESSION["user_id"];
	$response = $client->send_request($request);
		
	if($response == 0){
		echo "<h3> No Friends to display. </h3>";
		$error = date("Y-m-d") . "  " . date("h:i:sa") . "  --- Frontend --- " . "No friends to display." . "\n";
		log_event($error);
		...
		{				
			echo '<tr>';
				...
				echo '</td>';
				echo '<td class="p-3">';
					echo '<button type="button" class="bg-danger"><a class="btn-close" href="deletefriend.php?id=' . $value[1] . '&friend=' . $value[0] . '"></a></button>';
				echo '</td>';
			echo '</tr>';
		}
		echo '<table><br>';
	...
	echo "</div>";
	echo "<h3> <button class='bg-success text-light' type='button'><a class='bg-success text-light' href='search.php'>Click Here</a></button> to search a user to add. </h3>";
